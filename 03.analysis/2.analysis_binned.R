...
set.seed(1)
fit1 <- train(likes_bin2high ~ isMulti + weekday + period + hash_ct + mention_ct + emoji_ct,
              data=train,
...
train_all = cbind(model.matrix(likes_bin2high ~ isMulti + weekday + period + hash_ct + mention_ct + emoji_ct -1 ,
                          data=train),
...
set.seed(1)
fit3 <- train(y=train$likes_bin2high[samp2000],
              x=subtrain,
              method = 'ranger',
              importance='impurity',
              tuneGrid=rnggrid,
...
xgbgrid = expand.grid(nrounds=c(seq(50,1000,by=50)),
                      ...
                      gamma=0)
set.seed(1)
fit5 <- train(y=train$likes_bin3[samp2000],
              x=subtrain,
              method = 'xgbTree',
              verbose=TRUE,
              tuneGrid=xgbgrid,
...
set.seed(1)
fit6 <- train(y=train$likes_bin2[samp2000],
              x=subtrain,
              method = 'xgbTree',
              verbose=TRUE,
              tuneGrid=xgbgrid,
...
set.seed(1)
fit7 <- train(y=train$likes[samp2000]/train$followers[samp2000],
              x=subtrain,
              method = 'xgbTree',
              verbose=TRUE,
              tuneGrid=xgbgrid,
