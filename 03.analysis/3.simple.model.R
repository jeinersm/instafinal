...
###Linear fit with flags
 simple_fit2 <- train(likes ~ isMulti + weekday + period + hash_flag + followers +
                 mention_flag + emoji_flag,
               data=train,
               method = 'lm',
               trControl = fitControl)
 saveRDS(simple_fit2, file='./03.analysis/models/simple_scaled_ols_flags.rds')
 
 simple_fit2 = readRDS('./03.analysis/models/scaled_ols_flags.rds')

 simple_fit3 <- train(likes ~ isMulti + weekday + period + hash_flag + user +
                        mention_flag + emoji_flag,
                      data=train,
                      method = 'lm',
                      trControl = fitControl)
 saveRDS(simple_fit3, file='./03.analysis/models/simple_user_dummy.rds')
...
train_all = cbind(model.matrix(likes_scaled ~ isMulti + weekday + period + hash_ct + mention_ct + emoji_ct+followers -1 ,
                          data=train),
